...
<head>
    ...
    <title>FARM 2 MART - Seller Page</title>
    ...
    <style>
        * {
        ...
        .filter {
            ...
            padding: 10px 20px;
            box-shadow: 0px 0px 10px 1px rgba(122, 115, 115, 0.4);
            position: relative;
            bottom: 20px;
        }
...
<body>
    ...
    <main>
        <div class="firstDiv">
            ...
            <div class="welcomeSeller">WELCOME TO THE SELLER PAGE !!</div>
        </div>
        ...
        <div class="button-container">
            ...
            <p id="listOfBuyerName">LIST OF BUYERS</p>
            <a href="addRequirementAsSeller.jsp"><button class="add-requirement"> + ADD REQUIREMENT</button></a>
            <a href="manageUploads.jsp"><button class="manage-uploads"> + MANAGE UPLOADS</button></a>
        </div>
        
        <!-- Add select tags for state, district, product, and product type -->
        <div class="filter-container">
            <select class="filter" id="stateFilter" onchange="filterBuyers()">
                ...
                <%
                    try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/deepseek", "root", "AryanSql@1");
                         ...
                         ResultSet rs = stmt.executeQuery("SELECT DISTINCT state FROM Buyers")) {
                        while (rs.next()) {
            ...
            <select class="filter" id="districtFilter" onchange="filterBuyers()">
                <option value="">Select District</option>
                <!-- Add options dynamically from the database -->
                <%
                    try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/deepseek", "root", "AryanSql@1");
                         Statement stmt = conn.createStatement();
                         ResultSet rs = stmt.executeQuery("SELECT DISTINCT district FROM Buyers")) {
                        while (rs.next()) {
            ...
            <select class="filter" id="productFilter" onchange="filterBuyers()">
                <option value="">Select Product</option>
                <!-- Add options dynamically from the database -->
                <%
                    try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/deepseek", "root", "AryanSql@1");
                         Statement stmt = conn.createStatement();
                         ResultSet rs = stmt.executeQuery("SELECT DISTINCT product FROM Buyers")) {
                        while (rs.next()) {
            ...
            <select class="filter" id="productTypeFilter" onchange="filterBuyers()">
                <option value="">Select Product Type</option>
                <!-- Add options dynamically from the database -->
                <%
                    try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/deepseek", "root", "AryanSql@1");
                         Statement stmt = conn.createStatement();
                         ResultSet rs = stmt.executeQuery("SELECT DISTINCT productType FROM Buyers")) {
                        while (rs.next()) {
        ...
        <div class="container" id="buyersContainer">
            ...
            <%
                    ...
                    e.printStackTrace();
            %>
            <p>Error fetching buyers data!</p>
            <%
                }
            %>
        </div>
